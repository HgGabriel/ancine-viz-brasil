...
const Exhibition = () => {
  const { data: exhibitionData, isLoading: exhibitionLoading } = useExhibitionData();
  ...
  const avgTheatersPerComplex = totalComplexes > 0 ? (totalTheaters / totalComplexes) : 0;

  // Calculate additional metrics for better insights
  const largestState = exhibitionData?.salas_por_uf?.reduce((max: any, current: any) => 
    (current.total_salas > (max?.total_salas || 0)) ? current : max, null);
  const topExhibitionGroup = exhibitionData?.grupos_exibicao?.[0];

  // Prepare data for the Brazil map
  const mapData = exhibitionData?.salas_por_uf?.map((item: any) => ({
    uf: item.uf,
  ...
  const topMunicipalities = exhibitionData?.top_municipios?.slice(0, 10).map((item: any) => ({
    municipio: item.municipio,
    total_salas: item.total_salas
  })) || [];

  // Prepare data for exhibition groups chart
  const exhibitionGroups = exhibitionData?.grupos_exibicao?.slice(0, 10).map((item: any) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <KpiCard
          ...
          value={totalTheaters}
          icon={MapPin}
          description={`${largestState?.uf || 'N/A'}: ${largestState?.total_salas || 0} salas`}
          isLoading={exhibitionLoading}
        />
        <KpiCard
